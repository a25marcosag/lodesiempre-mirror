...
Añadidas búsquedas con filtro en la vista de listado de usuarios y en la vista de cada tienda para sus productos. En el footer se puede acceder a un nuevo popup del aviso y política de privacidad.

En el registro de usuario ahora hay que confirmar contraseña. Si había productos en el carrito temporal antes de iniciar sesión o registrarse, se añadirán al carrito real mientras se entre como usuario de tipo consumidor. Además, si es desde inicio de sesión se preguntará si se quieren añadir o no.

Ahora al comprar un carrito se envía un correo con los datos de este (a través del servicio de correos Resend), incluyendo el tipo de envío (este se elige en un nuevo selector).

También hay un botón de añadir/quitar verificación de la tienda del usuario. La edición de la imagen de tienda/producto ahora cuenta con una miniatura de la imagen actual.

Se corrigieron bugs y se mejoraron los estilos de la web, que ahora cuenta con animaciones desde la librería Animate.css.

# Funcionalidades conseguidas en el prototipo final

Todas las funcionalidades [listadas en la fase de análisis](doc/templates/2_analise.md) están ahora incorporadas.
