...
RUN apt-get update && apt-get install -y unzip curl git libzip-dev libonig-dev libsqlite3-dev libxml2-dev zlib1g-dev libicu-dev libpng-dev libjpeg-dev libfreetype6-dev \
...
RUN apt-get update && apt-get install -y libzip-dev libonig-dev libsqlite3-dev libxml2-dev zlib1g-dev libicu-dev libpng-dev libjpeg-dev libfreetype6-dev \
    && docker-php-ext-configure intl \
    && docker-php-ext-configure gd --with-freetype --with-jpeg \
    && docker-php-ext-install pdo_mysql pdo_sqlite mbstring zip exif pcntl bcmath opcache intl gd
