        ...
        <ul class="tarjetas" title="tarjetas">
            @foreach($usuarios as $u)
                <li class="tarjeta" title="tarjeta">
                    <p>
                        <form action="{{route('delete_usuario', ['id' => $u->id])}}" method="post" class="btn-borrar">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Seguro que quieres eliminar a este usuario junto con su tienda/carrito?')" aria-label="Eliminar a {{$u->nombre}}">
